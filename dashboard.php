...
// Seguran├ºa: apenas administradores
        ...
        <div class="sidebar">
            <div class="logo-area">
                <div class="logo">
                    ...
                    <div class="logo-text"><h2>CardioWeb</h2><p>Administra├º├úo</p></div>
                </div>
            </div>
            <nav class="nav-menu">
                ...
                <button class="nav-item <?php echo $page == 'agenda-medicos' ? 'active' : ''; ?>" onclick="loadContent('agenda-medicos')">
                    <i class="fas fa-calendar-alt"></i><span>Agenda dos M├®dicos</span>
                </button>
            </nav>
    ...
    <script>
        ...
        function carregarDados() {
            ...
            if (storedConsultas) {
                ...
                // Dados mockados iniciais (apenas se n├úo houver nada salvo)
                window.consultas = {
                    pendentes: [
        ...
        // ========== FUN├ç├òES DE APROVA├ç├âO/RECUSA ==========
        window.aprovarConsulta = function(id) {
            ...
            salvarDados();
            // Recarregar a se├º├úo para atualizar a UI
            loadContent('confirmar-consultas');
            alert('Ô£ô Consulta de ' + consulta.paciente + ' confirmada e adicionada ├á agenda!');
        };

        window.recusarConsulta = function(id) {
            const consulta = window.consultas.pendentes.find(c => c.id === id);
            if (!consulta) return;
            const mensagem = prompt('Digite a mensagem para o paciente (motivo da recusa ou sugest├úo de novo hor├írio):', 'Hor├írio indispon├¡vel. Sugerimos outro hor├írio.');
            ...
            salvarDados();
            loadContent('confirmar-consultas');
            alert('Ô£ë´©Å Mensagem enviada ao paciente: "' + mensagem + '"');
        };

        // ========== CARREGAR CONTE├ÜDO ==========
        const titles = {
            'confirmar-consultas': 'Confirmar Consultas',
            'agenda-medicos': 'Agenda dos M├®dicos'
        };
        ...
        function loadContent(section) {
            ...
            fetch(`admin-sections/${section}.php`)
                .then(response => {
                    if (!response.ok) throw new Error('Se├º├úo n├úo encontrada');
                    return response.text();
                })
                .then(html => {
                    contentArea.innerHTML = html;
                    // Disparar a renderiza├º├úo ap├│s o carregamento
                ...
                .catch(error => {
                    contentArea.innerHTML = `
                        <div class="info-card">
                            ...
                            <p style="color: #c00;">N├úo foi poss├¡vel carregar a se├º├úo. Verifique se o arquivo admin-sections/${section}.php existe.</p>
                        </div>
                    `;
                    console.error(error);
                });
